...
class Leader(State):
    def __init__(self):
        self._nextIndex = defaultdict(int)
    ...
    async def on_response_received(self, message):
        ...
        if not message.response:
            ...
            previousIndex = max(0, self._nextIndex[message.sender] - num_entries)
            ...
            current = self._server._log[self._nextIndex[message.sender]]
            ...
            if num_entries > 0:
                ...
                self._nextIndex[message.sender] = max(self._nextIndex[message.sender], self._matchIndex[message.sender] + 1)
